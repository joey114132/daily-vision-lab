...
export function classifyWeather(v: MoodVector): WeatherState {
  const candidates: { kind: WeatherKind; label: string; score: number }[] = [
    {
      kind: "golden-hour",
      label: "Golden hour",
      score: v.warmth * 1.2 - v.tension * 0.5 - v.drift * 0.2,
    },
    {
      kind: "high-pressure",
      label: "High pressure calm",
      score: (1 - v.tension) * 0.9 + (1 - v.drift) * 0.4 - v.surprise * 0.3,
    },
    {
      kind: "distant-storm",
      label: "Distant storm line",
      score: v.tension * 1.15 + v.drift * 0.25 - v.warmth * 0.35,
    },
    {
      kind: "sea-fog",
      label: "Sea fog",
      score: v.drift * 1.1 + (1 - v.warmth) * 0.35 - v.surprise * 0.2,
    },
    {
      kind: "aurora",
      label: "Aurora shear",
      score: v.surprise * 1.2 + v.warmth * 0.35 - v.tension * 0.15,
    },
    {
      kind: "heat-shimmer",
      label: "Heat shimmer",
      score: v.warmth * 0.7 + v.tension * 0.45 + v.surprise * 0.25,
    },
  ];
